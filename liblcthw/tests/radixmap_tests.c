...
static char *test_operations() {
	...
	mu_assert(make_random(map), "Din't make a random fake radix map.");

	RadixMap_sort(map);
	mu_assert(check_order(map),
	          "Failed to properlty sort the RadixMap"
	);
	...
	while(map->end > 0){
		...
		// but uint32 max so it trails of
		mu_assert(check_order(map), "Radix map didn't stay sorted after delete.");
	}
